...
# ============================================================
# client4_PFTL_share_shared_dense_gamma_logs_CLIENT6_PRINT_FIXED.py
# ============================================================
# PFTL: Share ONLY shared_dense
# - ONE SEED only (SEED), and SPLIT_SEED = SEED for train_test_split
# - SAME split: Test=20%, Val=10% of total (val_frac=0.125)
# - Correct class_weight mapping
# - NO plots
# - Safe bootstrap + strict barrier + one-row-per-round summary CSV
# ============================================================

# ---- Reproducible seeding (MUST be at top) ----
import os, random
SEED = 190
SPLIT_SEED = SEED  # explicit: same seed for splits
...
PRIVATE_DIM  = 4
SHARED_DIM   = 4
CONV_FILTERS = 4   # keep as-is; set to 4 if you want identical architecture
LR = 1e-3
...
# NEW: one row per round (macro-f1 only: local vs global)
...
class PFTLClientSharedDense:
    def __init__(self):
        ...
        # Bootstrap shared_dense from server (blend if valid)
        try:
    ...
    def _ack_ok(self, ack) -> bool:
        ...
        if hasattr(ack, "ok"):
            return bool(ack.ok)
        # 2) Ack(status="OK"/"WAITING"/"ERROR:..", current_round=..)
