...
# ============================================================
# aggregator_fedclassavg_clf_only_no_model.py
#
# FedClassAvg (Classifier-only FedAvg):
#   - SHARE/AGGREGATE ONLY: ["clf"]
#   - Everything else is PRIVATE on clients
#
...
SHARED_LAYER_NAMES = ["clf"]  # FedClassAvg shares only classifier head
...
class AggregatorService(myproto_pb2_grpc.AggregatorServicer):
    def __init__(self, num_clients=6):
        ...
        self.global_shared = None  # dict: {"clf": [W,b]}
        self.expected_shapes = None  # {"clf": [shapeW, shapeB]}

        print("[Aggregator] FedClassAvg init. Shared layers:", SHARED_LAYER_NAMES)
    ...
    def SendSharedUpdate(self, request, context):
        ...
        try:
            shared_dict = pickle.loads(request.weights)  # expected {"clf":[W,b]}
        ...
        with self.lock:
            ...
            print("[Aggregator] Aggregating classifier only:", SHARED_LAYER_NAMES)
...
if __name__ == "__main__":
    ...
    server.start()
    print(f"[Aggregator] Listening on :{PORT} | FedClassAvg (clf-only)")
