...
SHARED_LAYER_NAMES = ["clf"]  # share clf only
...
# -------------------------
# Shared get/set (clf only)
# -------------------------
def get_shared_clf(model):
    w = model.get_layer("clf").get_weights()
    if (not isinstance(w, (list, tuple))) or len(w) != 2:
        raise ValueError(f"[{CLIENT_ID}] clf weights invalid (expected [W,b]).")
...
def set_shared_clf(model, payload):
    ...
    if w is None or (not isinstance(w, (list, tuple))) or len(w) != 2:
