...
# ============================================================
# client1_FEDREP_share_representation_clf_private_TWO_STAGE.py
#
# FedRep (Representation shared, Head personalized):
# - Shared/Representation layers (send + receive):
#     input_adapter, feat1, shared_dense
# - Private/Personal head (never shared):
#     clf
#
# Key difference vs FedPer:
#   Each round uses TWO-STAGE training:
#     Stage A: train representation (freeze clf)
#     Stage B: train clf (freeze representation)
#
# Uses GAP for stable shapes
# Logs metrics + comm like your file
# ALSO saves: local vs global macro-F1 per round CSV
...
# FedRep two-stage epochs
REP_EPOCHS   = 1      # Stage A
HEAD_EPOCHS  = 1      # Stage B

BARRIER_POLL_SEC = 1.0

# ---- FedRep: share representation only (NO clf) ----
REP_LAYER_NAMES  = ["input_adapter", "feat1", "shared_dense"]
HEAD_LAYER_NAMES = ["clf"]  # private (not shared)
...
# -------------------------
def set_trainable_representation(model: tf.keras.Model, train_rep: bool):
    """
    If train_rep=True: representation trainable, clf frozen
    If train_rep=False: representation frozen, clf trainable
    """
    rep_set = set(REP_LAYER_NAMES)
    for layer in model.layers:
        if layer.name in rep_set:
            layer.trainable = bool(train_rep)
        if layer.name == "clf":
            layer.trainable = not bool(train_rep)

def compile_for_rep(model):
    model.compile(
        optimizer=optimizers.Adam(learning_rate=LR_REP, clipnorm=1.0),
...
def compile_for_head(model):
    model.compile(
        optimizer=optimizers.Adam(learning_rate=LR_HEAD, clipnorm=1.0),
        loss="binary_crossentropy",
        metrics=[tf.keras.metrics.BinaryAccuracy(name="accuracy")]
    )
...
class FedRepClient:
    def __init__(self):
        ...
        # default compile (we will recompile per stage anyway)
        ...
        # Bootstrap: pull global representation and set it (clf remains private)
        try:
            ...
            print(f"[{CLIENT_ID}] Bootstrapped representation from server round={resp.round} (clf private).")
        except Exception as e:
    ...
    def _personalize_head(self):
        """Head-only fitting after receiving global representation (FedRep)."""
        set_trainable_representation(self.model, train_rep=False)  # freeze rep, train clf
        compile_for_head(self.model)
        self.model.fit(
            self.Xtr, self.ytr,
            epochs=max(1, HEAD_EPOCHS),
    ...
    def run(self):
        ...
        for i in range(NUM_ROUNDS):
            print(f"\n[{CLIENT_ID}] ==== Round {i+1}/{NUM_ROUNDS} | current_round={self.current_round} ====")

            # -------- Stage A: train representation (freeze head) --------
            set_trainable_representation(self.model, train_rep=True)   # rep trainable, clf frozen
            compile_for_rep(self.model)
            ...
            # -------- Stage B: train head (freeze representation) --------
            set_trainable_representation(self.model, train_rep=False)  # rep frozen, clf trainable
            compile_for_head(self.model)
            ...
            local_mf1 = eval_and_log(self.model, self.Xte, self.yte, "local_before_sync", self.current_round)

            # Send ONLY representation layers (NO clf)
            payload_dict = get_layers_payload(self.model, REP_LAYER_NAMES)
            ...
            while True:
                ...
                if int(r_check) >= target:
                    ...
                    # apply global representation only
                    if isinstance(glob_w, dict):
                        set_layers_payload(self.model, glob_w)
                    else:
                    ...
                    # IMPORTANT in FedRep: re-personalize head after syncing representation
                    self._personalize_head()

                    global_mf1 = eval_and_log(self.model, self.Xte, self.yte, "global_after_sync", self.current_round)

                    # save one row per round: (server_round_after_sync, local_mf1, global_mf1)
        ...
        # save local vs global summary CSV
        save_local_global_csv(local_global_rows, LOCAL_GLOBAL_CSV)
        print(f"[{CLIENT_ID}] Saved local/global summary: {LOCAL_GLOBAL_CSV}")
