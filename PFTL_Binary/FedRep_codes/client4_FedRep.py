...
# ============================================================
# client4_FEDREP_repr_shared_head_private_logs_CLIENT6_PRINT.py
#
# FedRep:
#   - SHARE ONLY representation/backbone layers (FedAvg on server)
...
#
# Logs into SAME metrics CSV using phases:
#   - local_before_sync
#   - global_after_sync
#   - final
#
# - ONE SEED only (SEED), SPLIT_SEED = SEED
# - SAME split: Test=20%, Val=10% of total (val_frac=0.125)
# - Correct class_weight mapping {class_value: weight}
# - NO plots
...
#   - Aggregator should aggregate only representation layers it tracks
# ============================================================

# ---- Reproducible seeding (MUST be at top) ----
import os, random
SEED = 190
SPLIT_SEED = SEED  #  same seed for splits
...
HEAD_NAME = "clf"  # personalized head (never shared)
...
#  FedRep: only these are shared/averaged on server
# If you want FedRep without input_adapter, remove it here AND in aggregator.
...
def load_dataset():
    ...
    # 2) Val = 10% of total => 0.125 of train_full
...
# =========================
# Model Builder (CNN) - named layers
# =========================
...
# =========================
# FedRep payload helpers: share REPRESENTATION ONLY
# =========================
...
class FedRepClient4:
    def __init__(self):
        ...
        # Bootstrap: pull global representation layers and set them (NEVER overwrite clf)
        try:
    ...
    def run(self):
        ...
        for loop_i in range(NUM_ROUNDS):
            ...
            try:
                ...
                if int(resp.round) >= int(self.current_round):
                    self.current_round = int(resp.round)
            except Exception as e:
                print(f"[{CLIENT_ID}] Pull before train failed: {e}")

            # ----------------------------
            # A) REP TRAIN (freeze head)
            # ----------------------------
            compile_for_rep(self.model)
            ...
            # ----------------------------
            # STRICT barrier: wait server_round >= sent_round + 1
            # ----------------------------
            target = sent_round + 1
            ...
            # ----------------------------
            # B) HEAD TRAIN (freeze backbone) LOCAL ONLY
            # ----------------------------
