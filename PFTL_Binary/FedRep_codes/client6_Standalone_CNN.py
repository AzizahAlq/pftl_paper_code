# ========== Client 6 Standalone CNN (seeded; no padding='same') ==========
...
# (optional; helps determinism on CPU)
...
# =========================
# Load Dataset (Client 6)
# =========================
df = pd.read_csv("/Users/azizahalq/Desktop/FL_project3/Datasets_processed/D3_CICIOT2023/balanced_CICIDS2017.csv")
df.columns = df.columns.str.strip()

# Keep all features, drop only label column
...
# =========================
# Build CNN (no padding='same')
# =========================
model = Sequential([
    Input(shape=(n_features, 1)),
    Conv1D(8, 5, activation='relu'),   # default padding='valid'
    MaxPooling1D(pool_size=2),
    Flatten(),
    Dense(4, activation='relu'),                        # private
    Dense(4, activation='relu', name="shared_dense"),   # shared (for PTFL later),                        # private
...
conf_matrix = confusion_matrix(y_test, pred_test_bin)
...
plt.ylabel("Actual")
plt.title("Confusion Matrix - Client 6 (CICIDS_2017)")
plt.tight_layout()
plt.show()
...
plt.title("ROC Curves - Client 6 (CICIDS_2017)")
plt.legend()
plt.grid()
plt.tight_layout()
plt.show()
