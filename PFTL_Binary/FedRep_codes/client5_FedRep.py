...
# ============================================================
# client5_FEDREP_repr_shared_head_private_logs_CLIENT6_PRINT_NO_PLOTS.py
#
# FedRep:
...
#      -> wait barrier and pull aggregated representation
#   B) Train HEAD (clf) with representation frozen (local only, no sending)
#
...
# ONE SEED only, SPLIT_SEED = SEED for train_test_split
# NO plots
...
def compute_metrics(model, X, y_true):
    ...
    return {
        ...
        "prec1": prec1, "rec1": rec1, "f11": float(f11),
...
def log_metrics(phase, rnd, m):
    append_csv(METRICS_CSV, [
        ...
        round(m["prec1"], 6), round(m["rec1"], 6), round(float(m["f11"]), 6),
...
class FedRepClient5:
    def __init__(self):
        ...
        # Bootstrap: pull global representation and set it (NEVER overwrite clf)
        try:
    ...
    def run(self):
        ...
        for loop_i in range(NUM_ROUNDS):
            ...
            # (optional) pull latest representation before training
            try:
                resp = self.stub.GetSharedWeights(myproto_pb2.EmptyRequest(), metadata=[("client_id", CLIENT_ID)])
                if resp.weights:
