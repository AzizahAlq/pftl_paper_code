...
# ============================================================
# client4_FEDAVG_full_model_LOCAL_GLOBAL_SAME_CSV.py
#
# FedAvg (FULL MODEL sharing):
#   - SHARE: input_adapter, feat1, shared_dense, clf
#   - (pool/gap have no weights, so not sent)
#
...
#
# Strict barrier: wait until server_round >= sent_round + 1 (with timeout)
#
# Prints (Client6-style):
#   Accuracy, Macro/Weighted P/R/F1, classification_report, confusion matrix
# ============================================================
...
LAYERS_TO_SHARE = ["input_adapter", "feat1", "shared_dense", "clf"]
...
def set_layers_payload(model, payload):
    ...
    for ln in LAYERS_TO_SHARE:
        if ln not in payload:
            print(f"[{CLIENT_ID}] WARNING: missing '{ln}' in payload; skip.")
            return False
        if ln not in names:
            print(f"[{CLIENT_ID}] WARNING: '{ln}' not in local model; skip.")
            return False
        w = payload.get(ln)
        if w is None or (not isinstance(w, (list, tuple))) or len(w) == 0:
            print(f"[{CLIENT_ID}] WARNING: bad weights for '{ln}'; skip.")
            return False
        if any(np.isnan(np.asarray(x)).any() for x in w):
            print(f"[{CLIENT_ID}] WARNING: NaN in '{ln}' payload; skip.")
            return False
...
class FedAvgClient4Full:
        ...
        try:
            ...
            if resp.weights:
                ...
                if isinstance(payload, dict) and payload:
                    set_layers_payload(self.model, payload)
    ...
    def run(self):
        for loop_i in range(NUM_ROUNDS):
            ...
            print(f"\n[{CLIENT_ID}] ===== Loop {loop_i+1}/{NUM_ROUNDS} (server_round={sent_round}) =====")
            ...
            print_client6_style("TEST EVALUATION (local_before_sync)", m_local, self.yte)
            local_mf1 = float(m_local["mf1"])

            # send FULL payload
            try:
                payload_bytes = pickle.dumps(
                    get_layers_payload(self.model, LAYERS_TO_SHARE),
            ...
            while True:
                ...
                if int(resp.round) >= target:
                    ...
                    if isinstance(global_payload, dict) and global_payload:
                    ...
                    print_client6_style("TEST EVALUATION (global_after_sync)", m_global, self.yte)
                    global_mf1 = float(m_global["mf1"])

                    # summary row (after sync)
